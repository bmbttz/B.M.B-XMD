      ...
      const buttons = [
        ...
        },
        {
          ...
          buttonParamsJson: JSON.stringify({
            ...
            url: `https://whatsapp.com/channel/0029Vb2eknR59PwL1OK4wR24`
          })
        }
      ];
...
adams({ nomCom: "nature", reaction: "🦗", categorie: "B.M.B PICS" }, async (dest, zk, commandeOptions) => {
    const { repondre, arg, ms } = commandeOptions;
  const response = await fetch('https://api.unsplash.com/photos/random?client_id=72utkjatCBC-PDcx7-Kcvgod7-QOFAm2fXwEeW8b8cc');
const data = await response.json();
  const url =data.urls.regular
...
adams({ nomCom: "enhance", reaction: "💥", categorie: "User" }, async (dest, zk, commandeOptions) => {
    ...
    try {
      ...
      let caption = '*Enhanced by 𝙱.𝙼.𝙱-𝚇𝙼𝙳*';
